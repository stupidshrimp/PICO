...
| Commanded bank | `15 deg` | `flight_controller/loiter_nav.h` (`LOITER_BANK_ANGLE_DEG`), clamped into `FBW_MAX_ROLL_ANGLE_DEG`. Radius ≈ `v²/(g·tan θ)`, so about `30 m` at the `20 mph` auto-throttle default. The shallow bank also halves the lift shortfall the altitude loop has to fight, to about `3.4%` |
...
| Airspeed floor | `18 mph`, outranks altitude | `flight_controller/loiter_nav.h` (`LOITER_MIN_AIRSPEED_MPH`). A loop that pitches up to hold height while the throttle cannot sustain the climb flies the wing to a stall. Below the floor the hold is abandoned and pitch returns to **level**, which descends and regains speed — deliberately not a nose-down command, because level pitch is exactly the un-held orbit and therefore never worse than not having the feature. **This value is a placeholder below the auto-throttle default, not a measurement: set it above the airframe's measured clean stall speed before flying** |
...
| Altitude-hold degraded paths | all fall back to level pitch | No target captured (unusable barometer at engage), barometer stale or failing in flight, stale pitot, or airspeed below the floor. The orbit itself continues in every case; only the hold is abandoned |
...
| GS engage gates | transmitting, FBW, attitude fresh, joystick live, airborne **by both detectors** | `main.py` (`_loiter_gates`); the airborne check is what makes a hold on the ground fail *audibly* at the gesture instead of quietly raising CH10 for the FC to refuse |
| GS/FC airborne agreement | GS mirrors the FC's **latch**, engaging at `8 m/s` (`17.9 mph`) | `modules/loiter.py` (`fc_airborne_latched`, mirroring `AIRBORNE_ENGAGE_AIRSPEED_MPS`; the test suite reads that macro out of `Main.ino` so the mirror cannot drift). Airspeed only **sets** the flag — the firmware clears it on the disengage HEIGHT alone, and keeps orbiting below its airspeed floor by design, abandoning only altitude hold. Re-testing the engage threshold every cycle would therefore drop a running orbit the FC was happy to keep flying. The two detectors are independent and the GS one can be the **laxer**: with the default warning config it latches at `12 mph` (`stall_airspeed` `10` × `takeoff_airspeed_multiplier` `1.2`) while the FC waits for `17.9`. A hold in that window raises CH10, the FC refuses it as grounded, and the rising-edge rule means it stays refused even after the FC latches airborne — the orbit never flies while the operator has been told it is. Requiring the stricter of the two closes the window |
