...
| `2` | CH3 | Throttle / auto-throttle setpoint | Manual: throttle percent mapped to `172..1811`; Auto Throttle: desired airspeed mapped linearly over `0..100 mph`. The GS holds CH3 at minimum (`172`) for `0.4 s` after any throttle-mode change (`THROTTLE_MODE_CHANGE_GUARD_S` in `modules/auto_throttle.py`) and for the whole compass-cal request | Manual throttle percent, or FC auto-throttle target airspeed, selected by CH7. The two meanings share the same channel values, so a GS/FC mode disagreement makes the FC apply the wrong converter — a `20 mph` setpoint read as a manual percent is a steady `20%` throttle that ignores airspeed. CH7 is an AUX channel and ELRS sends AUX round-robin (roughly one per RF packet) while CH1–CH4 go out every packet, so CH7 reliably lands several packets after the CH3 value it selects for; the GS guard window covers that by sending minimum, which is safe under either converter (throttle cut, or a `0 mph` target) |
...
| FC auto-throttle derivative filter | `5 Hz` first-order, stepped at the pitot sample interval | `flight_controller/auto_throttle.h`, `flight_controller/Main.ino` (`AUTO_THROTTLE_DERIVATIVE_CUTOFF_HZ`); rejects pitot noise on the airspeed-rate term. The airspeed rate is differentiated against the `~60 Hz` pitot sample interval, not the `125 Hz` control period, so a constant acceleration yields a constant rate estimate instead of a sample-rate sawtooth that `Kd` would feed into the throttle command |
...
| FC auto-throttle PID gains | `Kp=0.8`, `Kd=0.15` (no `Ki`) | `flight_controller/auto_throttle.h` + `flight_controller/Main.ino` (`AUTO_THROTTLE_KP`/`KD`); output is integrated into the commanded throttle percent (velocity form), so the P term already provides integral action and a `Ki` would be a double integrator |
...
| Auto-throttle airspeed freshness timeout | `100000 us` (100 ms) | The FC closes the throttle loop only while pitot/airspeed data is fresh, independent of GPS lock | `flight_controller/Main.ino` (`AIRSPEED_FAILSAFE_TIMEOUT_US`) |
| Auto-throttle stale behavior | Decays by `50 percent/s` toward `0%` | If Auto Throttle is active but airspeed is stale, the FC forgets its airspeed sample history and ramps the commanded percent down toward idle; full RC loss still cuts throttle immediately. The GS shows this as `Auto <n> mph - NO AIRSPEED` on the throttle-mode indicator once pitot telemetry stops arriving (`main.py`, `auto_throttle_airspeed_live`), because the decay to idle is otherwise indistinguishable from "auto throttle does nothing" | `flight_controller/Main.ino` (`AUTO_THROTTLE_STALE_DECAY_PERCENT_PER_S`) |
| Auto-throttle command retention across RC gaps | Kept within the `500 ms` servo-hold window, dropped past it | Throttle is hard-cut for as long as RC is stale, so the FC keeps both the commanded throttle mode and the auto-throttle trim through a short gap and resumes from them — the commanded percent is the velocity form's only trim state, so zeroing it costs a full re-ramp from idle at `Kp*error percent/s`, and reverting to Manual Throttle would make re-engaging wait for a round-robin CH7 while CH3 was read as a manual percent. Past the servo-hold window the link is genuinely lost: the FC reverts to Manual Throttle and zeroes the trim so the aircraft cannot resume standing power | `flight_controller/Main.ino` (RC failsafe block, `rcServoHold`), `flight_controller/auto_throttle.h` (`resetHistory` vs `resetCommand`) |
