...
| Altitude-hold degraded paths | fall back to level pitch | No target captured (unusable barometer at engage), stale pitot, or airspeed below the floor. In these the orbit itself continues and only the hold is abandoned. **A stale barometer is the exception**: it also invalidates the airborne latch (row above), so the orbit ends entirely rather than continuing un-held — without a trustworthy altitude the FC cannot detect a landing at all |
