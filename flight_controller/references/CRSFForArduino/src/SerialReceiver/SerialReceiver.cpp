    ...
    void SerialReceiver::processFrames()
    {
        ...
        while (_uart->available() > 0)
        {
            ...
            if (crsf->receiveFrames(byteReceived))
            {
                ...
                const bool rcFrameDecoded = crsf->getRcChannels(_rcChannels->value);
                if (rcFrameDecoded && _rcChannelsCallback != nullptr)
                {
                ...
                {
                    /* Ensure the telemetry payload has been pushed out on the
                     * UART before we start reading the next downlink frame.
                     * Do not consume any RX bytes here—the remaining frames in
                     * the burst must be processed by the parser. */
                    _uart->flush();
                }
#endif
                setLinkUp();
