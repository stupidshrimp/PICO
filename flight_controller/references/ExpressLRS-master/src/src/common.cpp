...
uint8_t TLMBurstMaxForRateRatio(uint16_t const rateHz, uint8_t const ratioDiv)
{
    ...
    unsigned totalSlots = TELEM_MIN_LINK_INTERVAL_MS * rateHz / ratioDiv / 1000U;

    // Guarantee at least one advanced telemetry slot after the mandatory LINK_STATS frame.
    if (totalSlots < 2)
    {
        totalSlots = 2;
    }
