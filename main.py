...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.throttle_cfg.setdefault("airspeed_stale_timeout_s", 1.0)
        self.auto_throttle_airspeed_stale_timeout = float(
    ...
    def _make_configuration_scrollable(self) -> None:
        ...
        self.last_flight_metrics_packet_time = None
    ...
    def _update_auto_throttle_target(self) -> None:
        """Use fresh telemetry airspeed and a PID loop to choose throttle."""
        now = time.monotonic()
        if not self._has_fresh_airspeed(now):
            self._fail_safe_auto_throttle(now)
            return
        ...
        try:
            ...
            self._fail_safe_auto_throttle(now)
            return
        if not math.isfinite(current_airspeed):
            self._fail_safe_auto_throttle(now)
            return
    ...
    def _has_fresh_airspeed(self, now: Optional[float] = None) -> bool:
        """Return True when the cached airspeed came from recent GPS telemetry."""
        if self.last_airspeed_packet_time is None or self.current_airspeed is None:
            return False
        if now is None:
            now = time.monotonic()
    ...
    def _flight_metrics_value_fresh(self, now: float, timeout: float) -> bool:
        """Return True when pitot/baro flight metrics arrived recently.

        CRSF carries pitot airspeed and barometric altitude in the GPS telemetry
        frame used by this app, but those flight metrics are independent of the
        latitude/longitude fix.  Keep this separate from
        ``last_airspeed_packet_time`` so alarms can use fresh pitot/baro data
        without changing auto-throttle's stricter GPS-fix freshness policy.
        """

        if self.last_flight_metrics_packet_time is None:
            return False
        return (now - self.last_flight_metrics_packet_time) <= max(0.0, timeout)
    ...
    def _update_airborne_state(self, now: Optional[float] = None) -> None:
        ...
        # The packet type is named GPS in CRSF, but in this project it also
        # transports pitot airspeed and barometric altitude.  Airborne alarm
        # arming intentionally depends on fresh flight metrics, not on whether
        # the latitude/longitude portion currently has a GPS fix.
        if (
            not self._is_packet_fresh("gps", gps_timeout)
            or not self._flight_metrics_value_fresh(now, gps_timeout)
        ):
            self._airborne_takeoff_start_time = None
            self._airborne_landing_start_time = None
            return
    ...
    def _fail_safe_auto_throttle(self, now: Optional[float] = None) -> None:
        ...
        self.throttle_pid.reset()
        self.target_throttle_percent = 0
        self._last_auto_throttle_update = time.monotonic() if now is None else now
    ...
    def handle_telemetry(self, packet_type, *values) -> None:
        ...
        if packet_type == "attitude":
            ...
            # Airspeed and altitude come from pitot/baro sensors embedded in the
            # CRSF GPS telemetry frame.  They remain usable for alarm arming
            # even when the GPS receiver has not acquired a lat/lon fix, so use
            # a separate freshness timestamp that only requires finite flight
            # metrics and deliberately does not check ``gps_has_fix``.
            altitude_value = self._safe_float(alt)
            if (
                speed_value is not None
                and math.isfinite(speed_value)
                and altitude_value is not None
            ):
                self.last_flight_metrics_packet_time = now
            else:
                self.last_flight_metrics_packet_time = None
