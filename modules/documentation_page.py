...
class DocumentationPage:
    ...
    def _populate_gcs_tab(self, layout: QVBoxLayout) -> None:
        ...
        postflight_block = (
            ...
            "exercise — sticks never moved, no GPS fix — reports 'no data' rather than "
            "guessing."
        )
        ...
        self._add_subsection(
            ...
            (
                "<b>Telemetry continuity</b> &mdash; scans the attitude, GPS, and link streams for reception gaps and reports the effective packet rates. A gap over 1 second is a warning; over 5 seconds is a failure, because the aircraft was effectively invisible for that time.",
                ...
                "<b>Fly-By-Wire tracking</b> &mdash; the recorder now logs the control mode and the exact roll/pitch angles commanded in FBW, so this test grades every FBW segment directly: RMS tracking error and response lag, persistent offsets (trim/CG), ringing around the setpoint (over-tuned FC PID), attitude escaping the commanded envelope or the FC's hard angle ceiling, rough mode engagements, and time spent saturated at the angle limits.",
